...
fn main() {
	...
	let mut server = enetapi.create_host::<u32>(Some(&addr), 100, enet::ChannelLimit::Maximum, 
											enet::BandwidthLimit::Unlimited, enet::BandwidthLimit::Unlimited).unwrap();
...
{
	let event = server.service(1000);

	match &mut event.unwrap().unwrap()
	{
		Event::Connect(peer) =>
		{
			println!("Connection from peer! IP: {}", peer.address().ip().to_string());

			let id_local = *id;
			*id += 1;

			server.connect(&peer.address(), 4, id_local);
		...
		Event::Receive{sender, channel_id, packet} =>
		{
			...
			println!("Data: {} | from IP: {}", str, sender.address().ip().to_string());
		}
